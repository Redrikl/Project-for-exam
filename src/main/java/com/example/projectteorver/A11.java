...
public class A11 extends Namber{
    ...
    public String get(){
        return ". Рабочий обслуживает три станка. Вероятность того, " +
                "что в течение определенного промежутка времени откажет первый станок, равна "+vero+"; второй — "+vero1+"; третий — "+vero2+".\n" +
                "Составить ряд распределения числа станков, которые откажут в течение определенного промежутка времени. Найти М(Х), D(X), σ(X), F(X) этой случайной величины. Построить график F(X).\n";
    }
    public String aswer(){
